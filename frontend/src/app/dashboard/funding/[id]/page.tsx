...
function TypeBadge({ type }: { type: string }) {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs border font-medium ${map[type] ?? 'bg-white/10 text-white/50 border-white/20'}`}>
      {type.replace('_', ' ')}
    </span>
  )
}
...
export default function FundingDetailPage() {
  ...
  if (!agreement) return (
    <div className="flex flex-col items-center justify-center min-h-screen text-white/60 gap-4">
      <Banknote size={48} className="text-white/20" />
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto animate-fade-up">
      <Link href="/dashboard/funding" className="inline-flex items-center gap-2 text-white/40 hover:text-white/70 text-sm mb-6 transition-colors">
      ...
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-white">{agreement.title}</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white/5 border border-white/10 rounded-xl p-4">
          ...
          <p className="text-white font-semibold text-lg">{agreement.currency} {agreement.totalAmount.toLocaleString()}</p>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-xl p-4">
          <p className="text-white/40 text-xs mb-1">Spent</p>
          <p className="text-white font-semibold text-lg">{agreement.currency} {agreement.spent.toLocaleString()}</p>
          ...
          <p className="text-xs text-white/30 mt-1">{pct}% utilised</p>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-xl p-4">
          <p className="text-white/40 text-xs mb-1">Allocated to Projects</p>
          <p className="text-white font-semibold text-lg">{agreement.currency} {allocated.toLocaleString()}</p>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-xl p-4">
          <p className="text-white/40 text-xs mb-1">Expenses</p>
          <p className="text-white font-semibold text-lg">{agreement._count.expenses}</p>
        </div>
      </div>
      ...
      {agreement.donor && (
        <div className="bg-white/5 border border-white/10 rounded-xl p-5 mb-8">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-medium text-white/50 uppercase tracking-wide">Donor</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <p className="text-xs text-white/30 mb-0.5">Name</p>
              <p className="text-sm text-white/80">{agreement.donor.name}</p>
            </div>
            <div>
              <p className="text-xs text-white/30 mb-0.5">Organisation</p>
              <p className="text-sm text-white/80">{agreement.donor.organisationName}</p>
            </div>
            <div>
              <p className="text-xs text-white/30 mb-0.5">Type</p>
              <p className="text-sm text-white/80 capitalize">{agreement.donor.type.toLowerCase()}</p>
            </div>
            {agreement.donor.email && (
              <div>
                <p className="text-xs text-white/30 mb-0.5">Email</p>
                <p className="text-sm text-white/80">{agreement.donor.email}</p>
              </div>
            )}
          </div>
        </div>
      ...
      {(agreement.startDate || agreement.endDate || agreement.repayable || agreement.notes) && (
        <div className="bg-white/5 border border-white/10 rounded-xl p-5 mb-8">
          <h2 className="text-sm font-medium text-white/50 uppercase tracking-wide mb-3">Details</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {agreement.startDate && (
              <div>
                <p className="text-xs text-white/30 mb-0.5">Start Date</p>
                <p className="text-sm text-white/80">{new Date(agreement.startDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</p>
              </div>
            )}
            {agreement.endDate && (
              <div>
                <p className="text-xs text-white/30 mb-0.5">End Date</p>
                <p className="text-sm text-white/80">{new Date(agreement.endDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</p>
              </div>
            )}
            {agreement.repayable && (
              <div>
                <p className="text-xs text-white/30 mb-0.5">Interest Rate</p>
                <p className="text-sm text-white/80">{agreement.interestRate ?? 0}%</p>
              </div>
            )}
          </div>
          {agreement.notes && (
            <div className="mt-3">
              ...
              <p className="text-sm text-white/60">{agreement.notes}</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-white flex items-center gap-2">
            <FolderOpen size={18} className="text-white/40" /> Linked Projects
          </h2>
          <button onClick={() => setShowLinkProject(!showLinkProject)}
            className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm text-white/50 hover:text-white border border-white/10 hover:border-white/20 transition-all">
            <Plus size={14} /> Link Project
          </button>
        </div>

        {showLinkProject && (
          <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-4 flex items-end gap-3">
            <div className="flex-1">
              <label className="block text-xs text-white/30 mb-1">Project</label>
            ...
            <div className="w-40">
              <label className="block text-xs text-white/30 mb-1">Allocated Amount</label>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="text-left text-xs text-white/30 font-normal px-4 py-3">PROJECT</th>
                  <th className="text-left text-xs text-white/30 font-normal px-4 py-3">STATUS</th>
                  <th className="text-left text-xs text-white/30 font-normal px-4 py-3">ALLOCATED</th>
              ...
              <tbody>
                {agreement.projectFunding.map(pf => (
                  <tr key={pf.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className="text-xs text-white/50 capitalize">{pf.project.status}</span>
                    </td>
      ...
      {agreement.repayable && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-white flex items-center gap-2">
              <Calendar size={18} className="text-white/40" /> Repayment Schedule
            </h2>
          ...
          {showAddRepayment && (
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-4 flex items-end gap-3">
              <div className="flex-1">
                <label className="block text-xs text-white/30 mb-1">Due Date</label>
                <input type="date" value={repaymentDate} onChange={e => setRepaymentDate(e.target.value)} className={inputCls} />
              </div>
              <div className="w-40">
                <label className="block text-xs text-white/30 mb-1">Amount</label>
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
            {agreement.repayments.length === 0 ? (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/10">
                    <th className="text-left text-xs text-white/30 font-normal px-4 py-3">DUE DATE</th>
                    <th className="text-left text-xs text-white/30 font-normal px-4 py-3">AMOUNT</th>
                    <th className="text-left text-xs text-white/30 font-normal px-4 py-3">STATUS</th>
                    <th className="text-left text-xs text-white/30 font-normal px-4 py-3">PAID AT</th>
                  </tr>
                </thead>
                <tbody>
