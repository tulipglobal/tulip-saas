    ...
    <div className="p-6 space-y-6 animate-fade-up">
      ...
      {revealedKey && (
        <div className="rounded-xl border border-yellow-400/20 bg-yellow-400/5 px-5 py-4 space-y-2">
          ...
          <div className="flex items-center gap-2">
            <code className="text-xs text-[var(--tulip-forest)] bg-[var(--tulip-sage)] px-3 py-2 rounded-lg flex-1 overflow-x-auto">{revealedKey}</code>
            <button onClick={copyKey} className="px-3 py-2 rounded-lg bg-[var(--tulip-sage)] hover:bg-[var(--tulip-sage)] transition-colors">
              {copied ? <Check size={14} className="text-green-400" /> : <Copy size={14} className="text-[var(--tulip-forest)]/60" />}
            </button>
          </div>
          <button onClick={() => setRevealedKey(null)} className="text-xs text-[var(--tulip-forest)]/40 hover:text-[var(--tulip-forest)]/60">{t('dismiss')}</button>
        </div>
      )}
      ...
      <div className="flex items-center gap-3">
        <input
          ...
          className="bg-[var(--tulip-sage)] border border-[var(--tulip-sage-dark)] rounded-lg px-4 py-2.5 text-sm text-[var(--tulip-forest)] placeholder-[var(--tulip-forest)]/40 outline-none w-72"
          onKeyDown={e => e.key === 'Enter' && handleCreate()}
        />
        <button onClick={handleCreate} disabled={creating || !newKeyName.trim()}
          className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium text-[var(--tulip-forest)] disabled:opacity-40 bg-[var(--tulip-gold)] hover:bg-[var(--tulip-orange)]">
          <Plus size={16} />
      ...
      {/* Active keys */}
      <div className="rounded-xl border border-[var(--tulip-sage-dark)] overflow-hidden bg-[var(--tulip-sage)]">
        <div className="grid grid-cols-[2fr_1fr_1fr_1fr_80px] gap-4 px-5 py-3 border-b border-[var(--tulip-sage-dark)] text-xs text-[var(--tulip-forest)]/40 uppercase tracking-wide font-medium">
          <span>{t('name')}</span><span>{t('prefix')}</span><span>{t('lastUsed')}</span><span>{t('created')}</span><span />
        </div>
        {activeKeys.length === 0 ? (
          <div className="flex flex-col items-center py-16 gap-3">
            <Key size={32} className="text-[var(--tulip-forest)]/30" />
            <p className="text-[var(--tulip-forest)]/40 text-sm">{t('noActiveKeys')}</p>
          </div>
        ) : (
          <div className="divide-y divide-[var(--tulip-sage-dark)]">
            {activeKeys.map(k => (
              <div key={k.id} className="grid grid-cols-[2fr_1fr_1fr_1fr_80px] gap-4 items-center px-5 py-3.5 hover:bg-[var(--tulip-sage)]/50 transition-colors">
                <div>
                  <div className="text-sm font-medium text-[var(--tulip-forest)]">{k.name}</div>
                  <div className="text-xs text-[var(--tulip-forest)]/40 mt-0.5">{t('permissions', { count: k.permissions.length })}</div>
                </div>
                <code className="text-xs text-[var(--tulip-forest)]/60">{k.prefix}...</code>
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => handleRotate(k.id)} title="Rotate"
                    className="p-1.5 rounded-md text-[var(--tulip-forest)]/30 hover:text-yellow-400 hover:bg-yellow-400/5 transition-colors">
                    <RotateCw size={13} />
                  </button>
      ...
      {revokedKeys.length > 0 && (
        <div>
          <h2 className="text-sm font-medium text-[var(--tulip-forest)]/40 uppercase tracking-wide mb-3">{t('revokedKeys')}</h2>
          <div className="rounded-xl border border-[var(--tulip-sage-dark)] overflow-hidden bg-[var(--tulip-sage)]">
            <div className="divide-y divide-[var(--tulip-sage-dark)]">
              {revokedKeys.map(k => (
                <div key={k.id} className="flex items-center justify-between px-5 py-3 opacity-40">
                  <div>
                    <div className="text-sm text-[var(--tulip-forest)]/60 line-through">{k.name}</div>
                    <code className="text-xs text-[var(--tulip-forest)]/40">{k.prefix}...</code>
                  </div>
