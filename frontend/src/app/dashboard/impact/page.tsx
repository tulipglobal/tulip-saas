...
const CATEGORY_COLORS: Record<string, { bg: string; text: string }> = {
  ...
  'Sessions Conducted': { bg: '#e1eedd', text: '#183a1d' },
...
const STATUS_COLORS: Record<string, { bg: string; text: string }> = {
  ...
  'not_started': { bg: '#e1eedd', text: '#183a1d' },
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto" style={{ background: '#fefbe9', minHeight: '100%' }}>
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: '#f6c453' }}>
            <BarChart3 size={20} style={{ color: '#183a1d' }} />
          </div>
          <div>
            <h1 className="text-2xl font-bold" style={{ color: '#183a1d' }}>Impact Milestones</h1>
            <p className="text-sm" style={{ color: '#183a1d', opacity: 0.6 }}>Track the real-world impact of your projects</p>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 rounded-xl border" style={{ background: '#e1eedd', borderColor: '#c8d6c0' }}>
          <BarChart3 size={40} className="mx-auto mb-3" style={{ color: '#183a1d', opacity: 0.3 }} />
          <p className="text-sm" style={{ color: '#183a1d', opacity: 0.5 }}>No projects found. Create a project first to track milestones.</p>
        </div>
      ) : (
        <div className="space-y-8">
          {projects.map(project => {
            ...
            return (
              <div key={project.id} className="rounded-xl border overflow-hidden" style={{ borderColor: '#c8d6c0' }}>
                {/* Project Header */}
                <div className="flex items-center justify-between px-5 py-4" style={{ background: '#e1eedd' }}>
                  <h2 className="text-lg font-bold" style={{ color: '#183a1d' }}>{project.name}</h2>
                  <button
                ...
                {projectMilestones.length === 0 ? (
                  <div className="px-5 py-8 text-center">
                    <Target size={28} className="mx-auto mb-2" style={{ color: '#183a1d', opacity: 0.2 }} />
                    <p className="text-sm" style={{ color: '#183a1d', opacity: 0.4 }}>No milestones yet. Add your first milestone to start tracking impact.</p>
                  </div>
                ) : (
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start justify-between gap-2">
                            <h3 className="text-sm font-semibold" style={{ color: '#183a1d' }}>{ms.title}</h3>
                            <span
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-xs font-medium" style={{ color: '#183a1d', opacity: 0.6 }}>
                                {ms.currentValue} / {ms.targetValue} {ms.targetUnit}
                              </span>
                              <span className="text-xs font-bold" style={{ color: '#183a1d' }}>{pct}%</span>
                            </div>
                            <div className="w-full h-2 rounded-full" style={{ background: '#e1eedd' }}>
                              <div
                                className="h-2 rounded-full transition-all"
                                style={{
                                  width: `${pct}%`,
                                  background: pct >= 100 ? '#22c55e' : '#f6c453',
                                }}
                              />
                            </div>
                          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setAddModal(null)}>
          ...
          >
            <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: '#c8d6c0', background: '#e1eedd' }}>
              <h2 className="text-lg font-bold" style={{ color: '#183a1d' }}>Add Milestone</h2>
              <button onClick={() => setAddModal(null)} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-[#c8d6c0] transition-colors">
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: '#183a1d' }}>Category</label>
                <select
                  value={newCategory}
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: '#183a1d' }}>Title</label>
                <input
                  type="text"
                  value={newTitle}
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: '#183a1d' }}>Description</label>
                <textarea
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1.5" style={{ color: '#183a1d' }}>Target Value</label>
                  <input
                    type="number"
                    value={newTarget}
                ...
                <div>
                  <label className="block text-sm font-medium mb-1.5" style={{ color: '#183a1d' }}>Target Unit</label>
                  <input
                    type="text"
                    value={newUnit}
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: '#183a1d' }}>Target Date</label>
                <input
                  type="date"
                  value={newTargetDate}
      ...
      {updateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setUpdateModal(null)}>
          ...
          >
            <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: '#c8d6c0', background: '#e1eedd' }}>
              <h2 className="text-lg font-bold" style={{ color: '#183a1d' }}>Update Progress</h2>
              <button onClick={() => setUpdateModal(null)} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-[#c8d6c0] transition-colors">
                <X size={16} style={{ color: '#183a1d' }} />
              </button>
            </div>

            <div className="px-6 py-5 space-y-4">
              <div className="rounded-xl p-3" style={{ background: '#e1eedd', border: '1px solid #c8d6c0' }}>
                <p className="text-sm font-semibold" style={{ color: '#183a1d' }}>{updateModal.title}</p>
                <p className="text-xs mt-1" style={{ color: '#183a1d', opacity: 0.5 }}>
                  Current: {updateModal.currentValue} / {updateModal.targetValue} {updateModal.targetUnit}
                </p>
              </div>

              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: '#183a1d' }}>New Value</label>
                <input
                  type="number"
                  value={updateValue}
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: '#183a1d' }}>Note</label>
                <textarea
                  value={updateNote}
