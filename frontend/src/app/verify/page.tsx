...
async function generateCertificate(result: VerifyResult, fileName?: string) {
  ...
  y += 5
  doc.text('Sealayer — NGO Financial Transparency Platform — sealayer.io', 20, y)
  ...
  doc.setFontSize(7)
  doc.setTextColor(148, 163, 184)
  doc.text('Powered by Sealayer  •  Bright Bytes Technology  •  Dubai, UAE', W / 2, H - 8, { align: 'center' })

  doc.save(`sealayer-proof-${result.dataHash.slice(0, 12)}.pdf`)
}
...
async function generateProofBundle(result: VerifyResult, fileName?: string) {
  ...
  const verifyUrl = `https://sealayer.io/verify?hash=${result.dataHash}`
