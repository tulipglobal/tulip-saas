    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <div className="bg-[#e1eedd] border border-[#c8d6c0] rounded-xl p-4">
          <p className="text-[#183a1d]/60 text-xs mb-1">Total Risks</p>
          <p className="text-[#183a1d] font-semibold text-lg">{total}</p>
        </div>
        ...
        <div className="bg-[#e1eedd] border border-[#c8d6c0] rounded-xl p-4">
          <p className="text-[#183a1d]/60 text-xs mb-1">Open | Escalated</p>
          <p className="text-[#183a1d] font-semibold text-lg">{openCount} | {escalatedCount}</p>
        </div>
      </div>

      {/* Risk Matrix */}
      <div className="bg-[#e1eedd] border border-[#c8d6c0] rounded-xl p-6">
        <div className="flex items-center gap-2 mb-4">
          <Shield size={16} className="text-[#183a1d]" />
          <h3 className="text-sm font-semibold text-[#183a1d]">Risk Matrix</h3>
        </div>
        <div className="flex">
          {/* Y-axis label */}
          <div className="flex flex-col justify-between pr-2 py-1" style={{ writingMode: 'vertical-rl', transform: 'rotate(180deg)' }}>
            <span className="text-[10px] text-[#183a1d]/50 font-medium">Impact</span>
          </div>
          {/* Y-axis labels */}
          <div className="flex flex-col-reverse gap-1 pr-2 justify-between">
            {IMPACT_LABELS.map((label, idx) => (
              <div key={idx} className="h-12 flex items-center">
                <span className="text-[10px] text-[#183a1d]/60 whitespace-nowrap">{label}</span>
              </div>
            ))}
          </div>
          {/* Grid */}
                  ...
                  return (
                    ...
                    >
                      ...
                      {isHovered && cellRisks.length > 0 && (
                        <div className="absolute z-10 bottom-full mb-1 left-1/2 -translate-x-1/2 bg-[#183a1d] text-white text-[10px] rounded px-2 py-1 whitespace-nowrap shadow-lg max-w-[200px]">
                          {cellRisks.map((r, i) => (
            ...
            <div className="grid grid-cols-5 gap-1 mt-1">
              {LIKELIHOOD_LABELS.map((label, idx) => (
                <div key={idx} className="text-center">
                  <span className="text-[10px] text-[#183a1d]/60">{label}</span>
                </div>
              ))}
            </div>
            <p className="text-center text-[10px] text-[#183a1d]/50 font-medium mt-1">Likelihood</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-1 bg-[#e1eedd] p-1 rounded-lg">
          {FILTER_TABS.map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-1 rounded-md text-xs transition-all ${filter === f ? 'bg-white text-[#183a1d] shadow-sm' : 'text-[#183a1d]/60 hover:text-[#183a1d]'}`}>
              {f}
            </button>
          ))}
        </div>
      ...
      <div className="bg-[#e1eedd] border border-[#c8d6c0] rounded-xl overflow-hidden">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-[#183a1d]/40 gap-3">
            <AlertTriangle size={36} className="text-[#183a1d]/30" />
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#c8d6c0]">
                  ...
                  <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">CATEGORY</th>
                  <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">DESCRIPTION</th>
                  <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">L</th>
                  <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">I</th>
                  <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">SCORE</th>
                  <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">STATUS</th>
                  <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">MITIGATION</th>
                  <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">OWNER</th>
                  <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">REVIEW DATE</th>
              ...
              <tbody>
                {filtered.map((risk, idx) => {
                  ...
                  return (
                    <tr key={risk.id} className={`border-b border-[#c8d6c0] hover:bg-[#e1eedd]/50 transition-colors ${idx % 2 === 0 ? '' : 'bg-[#e1eedd]'}`}>
                      <td className="px-4 py-3 text-sm text-[#183a1d]/60">{globalIdx}</td>
                      <td className="px-4 py-3 text-xs text-[#183a1d]">{risk.category}</td>
                      <td className="px-4 py-3 text-sm text-[#183a1d] max-w-[200px] truncate">{risk.description}</td>
                      <td className="px-4 py-3 text-sm text-[#183a1d]/60">{risk.likelihood}</td>
                      <td className="px-4 py-3 text-sm text-[#183a1d]/60">{risk.impact}</td>
                      ...
                      <td className="px-4 py-3 text-xs text-[#183a1d]/60 max-w-[150px] truncate">{risk.mitigation}</td>
                      <td className="px-4 py-3 text-xs text-[#183a1d]/60">{risk.owner ?? '-'}</td>
                      <td className="px-4 py-3 text-xs text-[#183a1d]/60">{risk.reviewDate ? new Date(risk.reviewDate).toLocaleDateString() : '-'}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <button onClick={() => setEditingRisk(risk)} className="text-[#183a1d]/60 hover:text-[#183a1d] transition-colors" title="Edit">
                            <Edit2 size={14} />
                          </button>
                          <button onClick={() => handleDelete(risk.id)} className="text-[#183a1d]/60 hover:text-red-600 transition-colors" title="Delete">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </td>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-[#fefbe9] rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Category */}
          <div>
            <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Category</label>
          ...
          {/* Likelihood slider */}
          <div>
            <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">
            ...
            <div className="flex justify-between text-[9px] text-[#183a1d]/40 -mt-1">
              ...
              <div className="flex gap-2">
                {STATUS_OPTIONS.map(s => (
                  <button key={s} type="button" onClick={() => set('status', s)}
                    className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${form.status === s ? 'bg-[#f6c453] border-[#f6c453] text-[#183a1d] font-medium' : 'border-[#c8d6c0] text-[#183a1d]/60 hover:border-[#183a1d]/30'}`}>
                    {s}
                  </button>
                ))}
              </div>
          ...
          {/* Mitigation */}
          <div>
            <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Mitigation *</label>
          ...
          {/* Owner */}
          <div>
            <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Owner</label>
          ...
          {/* Review Date */}
          <div>
            <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Review Date</label>
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose}
              className="px-4 py-2 text-sm rounded-lg border border-[#c8d6c0] text-[#183a1d]/60 hover:text-[#183a1d] transition-all">
              Cancel
            </button>
            <button type="submit" disabled={saving}
              className="px-4 py-2 text-sm rounded-lg bg-[#f6c453] hover:bg-[#f0a04b] text-[#183a1d] font-medium transition-all disabled:opacity-50">
