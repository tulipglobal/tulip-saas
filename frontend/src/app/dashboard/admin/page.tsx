...
const EVENT_LABELS: Record<string, { label: string; color: string }> = {
  ...
  daily_active: { label: 'Active', color: 'bg-gray-100 text-gray-500' },
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 max-w-7xl">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Superadmin overview — customers, engagement, and hot leads</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl">
            <div className="p-4 border-b border-white/[0.06]">
              ...
              <div className="flex flex-wrap gap-2">
                <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5 flex-1 min-w-[180px]">
                  <Search size={12} className="text-gray-400" />
                  <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..."
                    className="bg-transparent text-xs text-gray-800 placeholder-gray-400 outline-none w-full" />
                </div>
                <select value={planFilter} onChange={e => setPlanFilter(e.target.value)}
                  className="bg-gray-50 border border-gray-200 rounded-lg px-2 py-1.5 text-xs text-gray-700 outline-none">
                ...
                <tbody className="divide-y divide-white/[0.04]">
                  {customers.length === 0 ? (
                    ...
                    <tr key={c.id} className="hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-4 py-3 text-gray-700 font-mono">{c.docsProcessed}</td>
                      <td className="px-4 py-3 text-gray-500">{new Date(c.signupDate).toLocaleDateString()}</td>
                      <td className="px-4 py-3 text-gray-500">{timeAgo(c.lastActive)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="space-y-6">
          ...
          {hotLeads.length > 0 && (
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl">
              <div className="p-4 border-b border-white/[0.06]">
                ...
                <p className="text-[10px] text-gray-400 mt-0.5">Free tenants with 3+ docs — ready to upgrade</p>
              </div>
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl">
            ...
            <div className="divide-y divide-white/[0.04]">
              {events.length === 0 ? (
                <div className="px-4 py-8 text-center text-gray-400 text-xs">No engagement events yet</div>
              ) : events.map(e => {
                const config = EVENT_LABELS[e.eventType] || { label: e.eventType, color: 'bg-gray-50 text-gray-500' }
                return (
                  <div key={e.id} className="px-4 py-3 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs text-gray-600 truncate">{e.tenantName}</div>
                    </div>
