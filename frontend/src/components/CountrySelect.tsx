...
export default function CountrySelect({ value, onChange, className = '', placeholder = 'Select country' }: CountrySelectProps) {
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      ...
      >
        <span className={value ? 'text-[var(--tulip-forest)]' : 'text-[var(--tulip-forest)]/40'}>{value || placeholder}</span>
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full rounded-lg border border-[var(--tulip-sage-dark)] bg-[var(--tulip-cream)] shadow-xl overflow-hidden">
          <div className="flex items-center gap-2 px-3 py-2 border-b border-[var(--tulip-sage-dark)]">
            ...
            />
            {search && (
              <button onClick={() => setSearch('')} className="text-[var(--tulip-forest)]/40 hover:text-[var(--tulip-forest)]">
                <X size={12} />
              </button>
            )}
          </div>
          <div className="max-h-52 overflow-y-auto">
            {filtered.length === 0 ? (
              <div className="px-3 py-4 text-center text-[var(--tulip-forest)]/40 text-xs">No countries found</div>
            ) : (
              filtered.map(country => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-sm hover:bg-[var(--tulip-sage)] transition-colors ${
                    value === country ? 'text-[var(--tulip-forest)] bg-[var(--tulip-gold)]/10 font-medium' : 'text-[var(--tulip-forest)]'
                  }`}
                >
                  {country}
                </button>
