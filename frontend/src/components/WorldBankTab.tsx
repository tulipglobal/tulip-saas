...
function methodPill(method: string) {
  return (
    <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-[#e1eedd] text-[#183a1d]/70 border border-[#c8d6c0]">
      {method}
    </span>
  )
}
    ...
    <div className="space-y-8">
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            <div className="flex items-center gap-2">
              <Globe size={16} className="text-[#183a1d]" />
              <h3 className="text-sm font-semibold text-[#183a1d]">Project Components</h3>
            </div>
            <p className="text-xs text-[#183a1d]/50 mt-0.5 ml-6">Budget breakdown by World Bank component</p>
          </div>
          <button onClick={() => setShowAddComponent(true)}
            className="flex items-center gap-1.5 text-sm font-medium text-[#183a1d] px-4 py-2 rounded-lg bg-[#f6c453] hover:bg-[#f0a04b] transition-all">
        ...
        <div className="bg-[#e1eedd] border border-[#c8d6c0] rounded-xl overflow-hidden">
          {components.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-[#183a1d]/40 gap-2">
              <Globe size={32} className="text-[#183a1d]/30" />
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[#c8d6c0]">
                    <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">COMPONENT</th>
                    <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">WB BUDGET</th>
                    <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">GOV BUDGET</th>
                    <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">ACTUAL THIS PERIOD</th>
                    <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">CUMULATIVE</th>
                    <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">CURRENCY</th>
                    <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">ACTION</th>
                  </tr>
                </thead>
                <tbody>
                  {components.map((comp, idx) => (
                    <tr key={comp.id} className={`border-b border-[#c8d6c0] hover:bg-[#e1eedd]/50 transition-colors ${idx % 2 === 0 ? '' : 'bg-[#e1eedd]'}`}>
                      <td className="px-4 py-3 text-sm text-[#183a1d]">{comp.name}</td>
                      <td className="px-4 py-3 text-sm text-[#183a1d] font-medium">{comp.wbBudget.toLocaleString()}</td>
                      <td className="px-4 py-3 text-sm text-[#183a1d]/60">{comp.govBudget.toLocaleString()}</td>
                      <td className="px-4 py-3 text-sm text-[#183a1d]/60">{comp.actualThisPeriod.toLocaleString()}</td>
                      <td className="px-4 py-3 text-sm text-[#183a1d]/60">{comp.cumulativeActual.toLocaleString()}</td>
                      <td className="px-4 py-3 text-xs text-[#183a1d]/60">{comp.currency}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <button onClick={() => setEditingActuals(comp)} className="text-[#183a1d]/60 hover:text-[#183a1d] transition-colors" title="Update Actuals">
                            <Edit2 size={14} />
                          </button>
                          <button onClick={() => handleDeleteComponent(comp.id)} className="text-[#183a1d]/60 hover:text-red-600 transition-colors" title="Delete">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </td>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <div>
            <div className="flex items-center gap-2">
              <FileText size={16} className="text-[#183a1d]" />
              <h3 className="text-sm font-semibold text-[#183a1d]">Procurement Contracts</h3>
            </div>
            <p className="text-xs text-[#183a1d]/50 mt-0.5 ml-6">Track contracts for World Bank reporting</p>
          </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[#c8d6c0]">
                    ...
                    <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">METHOD</th>
                    <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">EST COST</th>
                    <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">ACTUAL</th>
                    <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">STATUS</th>
                    <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">CONTRACT DATE</th>
                    <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">COMPLETION</th>
                    <th className="text-left text-xs text-[#183a1d]/40 font-normal px-4 py-3">ACTION</th>
                  </tr>
                </thead>
                <tbody>
                  {contracts.map((contract, idx) => (
                    <tr key={contract.id} className={`border-b border-[#c8d6c0] hover:bg-[#e1eedd]/50 transition-colors ${idx % 2 === 0 ? '' : 'bg-[#e1eedd]'}`}>
                      <td className="px-4 py-3 text-sm text-[#183a1d] max-w-[200px] truncate">{contract.description}</td>
                      <td className="px-4 py-3">{methodPill(contract.procurementMethod)}</td>
                      <td className="px-4 py-3 text-sm text-[#183a1d] font-medium">{contract.currency} {contract.estimatedCost.toLocaleString()}</td>
                      <td className="px-4 py-3 text-sm text-[#183a1d]/60">{contract.actualCost != null ? `${contract.currency} ${contract.actualCost.toLocaleString()}` : '-'}</td>
                      <td className="px-4 py-3">{contractStatusPill(contract.status)}</td>
                      <td className="px-4 py-3 text-xs text-[#183a1d]/60">{contract.contractDate ? new Date(contract.contractDate).toLocaleDateString() : '-'}</td>
                      <td className="px-4 py-3 text-xs text-[#183a1d]/60">{contract.completionDate ? new Date(contract.completionDate).toLocaleDateString() : '-'}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <button onClick={() => setEditingContract(contract)} className="text-[#183a1d]/60 hover:text-[#183a1d] transition-colors" title="Edit">
                            <Edit2 size={14} />
                          </button>
                          <button onClick={() => handleDeleteContract(contract.id)} className="text-[#183a1d]/60 hover:text-red-600 transition-colors" title="Delete">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </td>
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-[#fefbe9] rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-semibold text-[#183a1d]">{title}</h2>
          <button onClick={onClose} className="text-[#183a1d]/40 hover:text-[#183a1d]"><X size={18} /></button>
        </div>
        {children}
      </div>
    </div>
...
function ComponentForm({ onSave, onCancel }: { onSave: (data: any) => Promise<void>; onCancel: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Component Name *</label>
        <input required type="text" value={name} onChange={e => setName(e.target.value)}
          className="w-full rounded-lg border border-[#c8d6c0] bg-[#e1eedd] text-sm text-[#183a1d] px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#f6c453]/50" />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">WB Budget</label>
          <input type="number" step="0.01" value={wbBudget} onChange={e => setWbBudget(e.target.value)}
            className="w-full rounded-lg border border-[#c8d6c0] bg-[#e1eedd] text-sm text-[#183a1d] px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#f6c453]/50" />
        </div>
        <div>
          <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Gov Budget</label>
          <input type="number" step="0.01" value={govBudget} onChange={e => setGovBudget(e.target.value)}
            className="w-full rounded-lg border border-[#c8d6c0] bg-[#e1eedd] text-sm text-[#183a1d] px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#f6c453]/50" />
        </div>
      </div>
      <div>
        <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Currency</label>
      ...
      <div className="flex justify-end gap-3 pt-2">
        <button type="button" onClick={onCancel} className="px-4 py-2 text-sm rounded-lg border border-[#c8d6c0] text-[#183a1d]/60 hover:text-[#183a1d] transition-all">Cancel</button>
        <button type="submit" disabled={saving} className="px-4 py-2 text-sm rounded-lg bg-[#f6c453] hover:bg-[#f0a04b] text-[#183a1d] font-medium transition-all disabled:opacity-50">
...
function ActualsForm({ initial, onSave, onCancel }: { initial: WBComponent; onSave: (data: any) => Promise<void>; onCancel: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Actual This Period</label>
        <input type="number" step="0.01" value={actualThisPeriod} onChange={e => setActualThisPeriod(e.target.value)}
          className="w-full rounded-lg border border-[#c8d6c0] bg-[#e1eedd] text-sm text-[#183a1d] px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#f6c453]/50" />
      </div>
      <div>
        <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Cumulative Actual</label>
...
function ContractForm({ onSave, onCancel }: { onSave: (data: any) => Promise<void>; onCancel: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Description *</label>
        <input required type="text" value={description} onChange={e => setDescription(e.target.value)}
          className="w-full rounded-lg border border-[#c8d6c0] bg-[#e1eedd] text-sm text-[#183a1d] px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#f6c453]/50" />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Procurement Method *</label>
        ...
        <div>
          <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Estimated Cost *</label>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Contract Date</label>
          <input type="date" value={contractDate} onChange={e => setContractDate(e.target.value)}
            className="w-full rounded-lg border border-[#c8d6c0] bg-[#e1eedd] text-sm text-[#183a1d] px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#f6c453]/50" />
        </div>
      </div>
      <div>
        <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Notes</label>
...
function ContractUpdateForm({ initial, onSave, onCancel }: { initial: WBContract; onSave: (data: any) => Promise<void>; onCancel: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Actual Cost</label>
        <input type="number" step="0.01" value={actualCost} onChange={e => setActualCost(e.target.value)}
          className="w-full rounded-lg border border-[#c8d6c0] bg-[#e1eedd] text-sm text-[#183a1d] px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#f6c453]/50" />
      </div>
      <div>
        <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Status</label>
      ...
      <div>
        <label className="text-xs font-medium text-[#183a1d]/70 block mb-1">Completion Date</label>
