          ...
          {allProjects.map(p => (
            <label
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg border cursor-pointer transition-all ${
                selected.includes(p.id) ? 'border-[var(--tulip-gold)] bg-[var(--tulip-gold)]/10' : 'border-[var(--tulip-sage-dark)] hover:border-[var(--tulip-gold)]/50'
              }`}
            >
              <input type="checkbox" checked={selected.includes(p.id)} onChange={() => toggle(p.id)} className="accent-[var(--tulip-gold)] w-4 h-4" />
    ...
    <div className="p-6 space-y-6 animate-fade-up max-w-4xl">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href="/dashboard/settings" className="text-[var(--tulip-forest)]/40 hover:text-[var(--tulip-forest)] transition-all">
            <ArrowLeft size={18} />
          </Link>
          <div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-3">
                {donors.map(donor => {
                  const activeProjects = donor.projects.filter(p => p.active)
                  return (
                    <div key={donor.memberId} className="rounded-xl border border-[var(--tulip-sage-dark)] bg-[var(--tulip-sage)] overflow-hidden">
                      <div className="flex items-center justify-between px-4 py-3">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="relative shrink-0">
                            <div className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold text-[var(--tulip-forest)] bg-[var(--tulip-gold)]">
                              {(donor.name || donor.email).charAt(0).toUpperCase()}
                            </div>
                            <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-[var(--tulip-sage)] ${onlineUsers.has(donor.orgId) ? 'bg-green-400' : 'bg-gray-300'}`} />
                          </div>
                        ...
                        <div className="px-4 pb-3">
                          <div className="flex flex-wrap gap-1.5">
                            {activeProjects.map(p => (
                              <span key={p.projectId} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-[var(--tulip-gold)]/15 text-[var(--tulip-forest)]/70">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {pendingInvites.map(inv => (
                      <tr key={inv.id} className="border-b border-[var(--tulip-sage-dark)]/50 last:border-0">
                        <td className="px-4 py-3 font-medium text-[var(--tulip-forest)]">{inv.donorOrgName || '—'}</td>
                        <td className="px-4 py-3 text-[var(--tulip-forest)]/70">{inv.email}</td>
                        <td className="px-4 py-3 text-[var(--tulip-forest)]/50 text-xs">
