...
const inputCls = "w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-white/30 outline-none focus:border-[#0c7aed]/50 transition-all [&>option]:bg-[#0a1628]"
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-6 animate-fade-up">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {editingStatus ? (
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => { setEditingStatus(false); setStatusError('') }} className="text-white/30 hover:text-white/50"><X size={14} /></button>
              </div>
            ) : (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Budget Required', value: `$${budget.totalApproved.toLocaleString()}`, color: 'text-white' },
          ...
          <div className="divide-y divide-white/5">
            {budget.fundingSources.map(f => (
              <div key={f.id} className="flex items-center justify-between px-5 py-3">
                <div>
                  <div className="text-sm text-white/70">{f.donorName}</div>
                  <div className="text-xs text-white/30">
                    {f.sourceType}{f.sourceSubType ? ` / ${f.sourceSubType}` : ''}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium text-white">{f.currency} {f.amount.toLocaleString()}</span>
        ...
        {showAddFunding && (
          <div className="px-5 py-4 border-t border-white/8 space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setShowAddFunding(false)} className="px-4 py-1.5 text-xs text-white/40 hover:text-white/60">Cancel</button>
            </div>
          </div>
        )}
