...
import { CAPEX_CATEGORIES, OPEX_CATEGORIES, getCategoriesForType } from '@/lib/ngo-categories'
...
export default function NewBudgetPage() {
  ...
  const updateLine = (key: string, field: keyof BudgetLineForm, value: string) => {
    setLines(prev => prev.map(l => {
      ...
      // Reset category/subCategory when expenseType changes
  ...
  const totalCapex = lines
    .filter(l => l.expenseType === 'CAPEX')
    .reduce((s, l) => s + (Number(l.approvedAmount) || 0), 0)
  const totalOpex = lines
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            ...
            <input value={name} onChange={e => setName(e.target.value)}
              ...
              className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-white/30 outline-none focus:border-[#0c7aed]/50 transition-all" />
          </div>
          <div>
            <label className="text-xs text-white/40 mb-1 block">Period From *</label>
            <input type="date" value={periodFrom} onChange={e => setPeriodFrom(e.target.value)}
              className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white outline-none focus:border-[#0c7aed]/50 transition-all [color-scheme:dark]" />
          </div>
          <div>
          ...
          <div className="md:col-span-2">
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2}
              ...
              className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-white/30 outline-none focus:border-[#0c7aed]/50 transition-all resize-none" />
          </div>
        </div>
      </div>
