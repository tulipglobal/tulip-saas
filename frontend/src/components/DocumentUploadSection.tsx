  ...
  return (
    <div className="rounded-xl border border-white/8 p-5 space-y-4"
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm font-medium text-white/70">Attach Document <span className="text-white/30 text-xs">(optional)</span></span>
      </div>
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition-all ${
          dragOver ? 'border-[#0c7aed] bg-[#0c7aed]/5' : file ? 'border-green-500/40 bg-green-500/5' : 'border-white/10 hover:border-white/20'
        }`}
      >
        ...
        {file ? (
          ...
          <div>
            ...
            <p className="text-sm text-white/40">Drop file here or <span className="text-[#369bff]">browse</span></p>
            <p className="text-xs text-white/20 mt-1">PDF, Word, Excel, Image — max 20MB</p>
          </div>
        )}
      </div>
