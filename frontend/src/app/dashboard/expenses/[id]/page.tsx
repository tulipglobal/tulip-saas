...
export default function ExpenseDetailPage() {
  ...
  if (error || !expense) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[var(--tulip-cream)] text-[var(--tulip-forest)]/70 gap-4">
      ...
      <Link href="/dashboard/expenses" className="text-[var(--tulip-forest)] hover:text-[var(--tulip-gold)] text-sm">← Back to Expenses</Link>
    </div>
  )
    ...
    <div className="min-h-screen bg-[var(--tulip-cream)] text-[var(--tulip-forest)] p-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-[var(--tulip-sage)] border border-[var(--tulip-sage-dark)] rounded-xl p-4">
          <p className="text-[var(--tulip-forest)]/60 text-xs mb-1">Amount</p>
          <p className="text-[var(--tulip-forest)] font-semibold text-lg">{expense.currency} {expense.amount.toLocaleString()}</p>
        </div>
        <div className="bg-[var(--tulip-sage)] border border-[var(--tulip-sage-dark)] rounded-xl p-4">
          <p className="text-[var(--tulip-forest)]/60 text-xs mb-1">Project</p>
          {expense.project ? (
        ...
        <div className="bg-[var(--tulip-sage)] border border-[var(--tulip-sage-dark)] rounded-xl p-4">
          <p className="text-[var(--tulip-forest)]/60 text-xs mb-1">Funding Source</p>
          <p className="text-[var(--tulip-forest)] text-sm">{expense.fundingSource?.name ?? '—'}</p>
        </div>
        <div className="bg-[var(--tulip-sage)] border border-[var(--tulip-sage-dark)] rounded-xl p-4">
          <p className="text-[var(--tulip-forest)]/60 text-xs mb-1">Documents</p>
      ...
      {expense.dataHash && (
        <div className="bg-[var(--tulip-sage)] border border-[var(--tulip-sage-dark)] rounded-xl p-4 mb-8">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[var(--tulip-forest)]/60 text-xs mb-1 flex items-center gap-1"><Hash size={11} /> Data Hash (SHA-256)</p>
      ...
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-lg font-semibold text-[var(--tulip-forest)]">Documents</h2>
      ...
      <div className="bg-[var(--tulip-sage)] border border-[var(--tulip-sage-dark)] rounded-xl overflow-hidden">
        {docs.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-[var(--tulip-forest)]/40 gap-3">
            ...
            <p className="text-xs text-[var(--tulip-forest)]/30">Upload receipts, invoices, or supporting documents</p>
          </div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-[var(--tulip-sage-dark)]">
                ...
                <th className="text-left text-xs text-[var(--tulip-forest)]/40 font-normal px-4 py-3">SIZE</th>
                <th className="text-left text-xs text-[var(--tulip-forest)]/40 font-normal px-4 py-3">SHA-256 HASH</th>
