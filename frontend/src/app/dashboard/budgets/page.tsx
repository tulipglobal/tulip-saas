        ...
        {loading ? (
          ...
          <div className="divide-y divide-[#c8d6c0]">
            {filtered.map(b => {
              ...
              return (
                <div key={b.id} className="px-4 py-3.5 hover:bg-[#e1eedd]/50 transition-colors lg:grid lg:grid-cols-[2fr_1fr_1fr_1fr_1fr_1.5fr] lg:gap-4 lg:items-center lg:px-5">
                  <div>
                    <Link href={`/dashboard/budgets/${b.id}`} className="text-sm font-medium text-[#183a1d] hover:text-cyan-400 transition-colors">
                      {b.name}
                    </Link>
                    <div className="text-xs text-[#183a1d]/40 mt-0.5">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-2 lg:hidden">
                      ...
                      <span className="text-xs text-[#183a1d]/60">{formatDate(b.periodFrom)} – {formatDate(b.periodTo)}</span>
                      <span className="text-sm font-medium text-[#183a1d]">${b.totalApproved.toLocaleString()}</span>
                    </div>
