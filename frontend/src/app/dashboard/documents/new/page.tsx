  ...
  if (success) return (
    <div style={{ minHeight:'100vh', background:'#0f172a', display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ textAlign:'center' }}>
        ...
        <p style={{ color:'white', fontSize:20, fontWeight:600 }}>Document uploaded successfully</p>
    ...
    <div style={{ minHeight:'100vh', background:'#0f172a', padding:'32px' }}>
      <div style={{ maxWidth:720, margin:'0 auto' }}>
        <div style={{ marginBottom:32 }}>
          ...
          <h1 style={{ color:'white', fontSize:28, fontWeight:700, margin:0 }}>Add Document</h1>
          <p style={{ color:'#64748b', fontSize:14, marginTop:4 }}>SHA-256 hash will be generated and anchored to blockchain</p>
        </div>

        <div style={{ marginBottom:24 }}>
          <p style={{ color:'#94a3b8', fontSize:12, fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:12 }}>Document Level</p>
        ...
        {(level === 'project' || level === 'expense') && (
          <div style={{ marginBottom:16 }}>
            <label style={{ color:'#94a3b8', fontSize:12, fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', display:'block', marginBottom:8 }}>Project *</label>
        ...
        {level === 'expense' && selectedProject && (
          <div style={{ marginBottom:16 }}>
            <label style={{ color:'#94a3b8', fontSize:12, fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', display:'block', marginBottom:8 }}>Expense *</label>
        ...
        <div style={{ marginBottom:16 }}>
          <label style={{ color:'#94a3b8', fontSize:12, fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', display:'block', marginBottom:8 }}>Document Type</label>
        ...
        <div style={{ marginBottom:24 }}>
          <label style={{ color:'#94a3b8', fontSize:12, fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', display:'block', marginBottom:8 }}>Description</label>
          <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} placeholder="Brief description..."
            style={{ width:'100%', padding:'10px 14px', borderRadius:8, border:'1px solid rgba(255,255,255,0.1)', background:'rgba(255,255,255,0.05)', color:'white', fontSize:14, resize:'vertical', boxSizing:'border-box' }} />
        </div>
          ...
          <div onDragOver={e => { e.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={handleDrop} onClick={() => fileRef.current?.click()}
            ...
            {file ? (
              ...
              <div>
                ...
                <p style={{ color:'#94a3b8', fontWeight:500, margin:0 }}>Drop file here or click to browse</p>
        ...
        <div style={{ display:'flex', gap:12 }}>
          <Link href="/dashboard/documents" style={{ flex:1, padding:'12px', borderRadius:8, border:'1px solid rgba(255,255,255,0.1)', color:'#94a3b8', fontSize:14, fontWeight:500, textAlign:'center', textDecoration:'none', display:'block' }}>Cancel</Link>
          <button onClick={handleSubmit} disabled={loading}
            style={{ flex:2, padding:'12px', borderRadius:8, background: loading ? '#1e293b' : '#0d9488', color:'white', fontSize:14, fontWeight:600, border:'none', cursor: loading ? 'not-allowed' : 'pointer' }}>
