...
async function generateOcrPdf(doc, assessment, jobId) {
  return new Promise((resolve, reject) => {
    ...
    pdf.fillColor(colors.muted).fontSize(7).font('Helvetica')
      .text(
        `Generated by Sealayer · sealayer.io · ${new Date().toISOString()} · This document has been processed using AI-assisted OCR and normalisation. Original document preserved.`,
...
async function generateBundlePdf(bundleName, crossAnalysis, documents, bundleId) {
  return new Promise((resolve, reject) => {
    ...
    pdf.fillColor(colors.muted).fontSize(7).font('Helvetica')
      .text(
        `Generated by Sealayer · sealayer.io · ${new Date().toISOString()} · Cross-document verification performed using AI analysis. Individual documents preserved.`,
        50, pdf.y, { width: pdf.page.width - 100, align: 'center' }
      )
